...
try{
 ...
 if($action==='join' && $_SERVER['REQUEST_METHOD']==='POST'){
   ...
   if(!$t){$pdo->rollBack();out(['ok'=>false,'error'=>'This trip is no longer available.'],404);}if($t['joined']+$people>$t['capacity']){$pdo->rollBack();out(['ok'=>false,'error'=>'Not enough spaces remain for this trip.'],409);}
   $q=$pdo->prepare('INSERT INTO participants(trip_id,full_name,phone,email,people,message) VALUES(?,?,?,?,?,?)');try{$q->execute([$trip,$name,$phone,$email,$people,$message]);}catch(PDOException $e){$pdo->rollBack();if((int)$e->errorInfo[1]===1062)out(['ok'=>false,'error'=>'This email is already registered for this trip.'],409);throw $e;}$pdo->commit();out(['ok'=>true,'message'=>'Registration confirmed.']);
 }
